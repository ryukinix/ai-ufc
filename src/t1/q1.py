...
def hill_climbing(f, interval=(0, 20), max_steps=1000):
    node = randint(*interval), randint(*interval)
    ...
    best_x = round(best_node[0], ndigits=1)
    best_y = round(best_node[1], ndigits=1)
    best_value = round(best_value, ndigits=1)
    start_value = round(start_value, ndigits=1)
